...
{	
	...
	content.saveConfig = function()
	{
		...
		UI.InfoMessage('Nastavení uloĹľeno.',3000,'success');
	};
	...
	content.nameVillages = function()
	{
		...
		{
			UI.InfoMessage('Názvy jsou pĹ™íliĹˇ dlouhĂ©! (max 32 znakĹŻ).',5000,'error');
		}				
		else
		{
		...
		for(var i=0;i<options.length;i++)
		{
			...
			if(options[i].name == 'number_villages') //put the special numbering input in the label spot
			{
				...
				numberingInputTable.rows[0].cells[1].innerHTML = 'ÄŚíslice';
		...
		modeForm.id = 'twcheese_name_mode_form';
			...
			modeForm.innerHTML += 'PĹ™epsat aktuální názvy';
			...
			modeForm.innerHTML += 'PĹ™idat pred souÄŤasný název';			
			...
			modeForm.innerHTML += 'PĹ™idat za souÄŤasný název';
	...
	twcheese.renameVillages = function(config,mode)
	{
		...
		$('.quickedit-vn').each(function(key,village){ //each village
			...
			if (name.length <= 32) {
				...
				UI.InfoMessage('Název je pĹ™íliĹˇ dlouhý (max. 32 znakĹŻ).<br/>Nový název nebude aplikován na vĹˇechny vesnice.',5000,'error');
			}
		...
		window.setTimeout(
			function(){
				...
				if(nameSaveButtons.length > 0){
					...
					UI.InfoMessage('Názvy vesnic byly úspÄ›ĹˇnÄ› zmÄ›nÄ›ny.',5000,'success');
				}
			},
			saveDelay
		);
...
/*==== main ====*/
	...
	if (game_data.screen == 'overview_villages' || canNameVillages) {
		...
		UI.InfoMessage('PouĹľijte v náhledu vesnic.',5000,'error');
	}
